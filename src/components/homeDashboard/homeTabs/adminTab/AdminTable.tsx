...
const AdminTable = ({
  ...
  const filteredAndSortedAdmins = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        (admin) =>
          admin.first_name?.toLowerCase().includes(lowerSearch) ||
          admin.last_name?.toLowerCase().includes(lowerSearch) ||
          admin.email?.toLowerCase().includes(lowerSearch) ||
          admin.school_email?.toLowerCase().includes(lowerSearch)
      );
    }
  ...
  const handleRowClick = (admin: UserType) => {
    if (onCardClick) {
      onCardClick(admin);
    }
  };
  ...
  if (error) {
    return (
      <Box p={8} textAlign="center">
        <Text color="red.500">Error loading admins: {error.message}</Text>
      </Box>
    );
  }
        ...
        <Table.Body>
          {filteredAndSortedAdmins.map((admin, index) => (
            ...
            >
              <Table.Cell p={3}>
                <Image
                  src={admin.profile_picture_url || profileDefaultIcon}
              ...
              <Table.Cell p={3}>
                <Badge
                  colorPalette={admin.is_active ? "green" : "red"}
                  size="sm"
                >
